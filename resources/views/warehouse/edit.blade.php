<x-layout title="Ubah Gudang">
    <div class="space-y-6">
        ...
        <div class="grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
            ...
            <form method="POST" action="{{ route('warehouse.update', $warehouse) }}" class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                @method('PUT')
                @csrf
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <div class="mt-2">
                            <textarea id="address" name="address" rows="5"
                                ...
                                placeholder="Masukkan alamat gudang">{{ old('address', $warehouse->address) }}</textarea>
                            @error('address')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <div class="mt-4 space-y-4 text-sm">
                    ...
                    <div class="rounded-lg bg-gray-50 px-3 py-3">
                        ...
                        <p class="mt-1 font-medium text-gray-700">{{ $warehouse->updated_at ? $warehouse->updated_at->format('d M Y, H:i') : '-' }}</p>
                    </div>
                </div>
            </div>
        </div>
